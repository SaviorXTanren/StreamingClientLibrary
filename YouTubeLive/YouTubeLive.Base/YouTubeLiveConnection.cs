...
using YouTubeLive.Base.Services;

[assembly: InternalsVisibleTo("YouTubeLive.Base.UnitTests")]

namespace YouTubeLive.Base
{
    /// <summary>
    ...
    /// <summary>
    /// The main connection handler for YouTube Live.
    /// </summary>
    public class YouTubeLiveConnection
    {
        /// <summary>
        ...
        public static async Task<string> GetAuthorizationCodeURLForOAuthBrowser(string clientID, IEnumerable<OAuthClientScopeEnum> scopes, string redirectUri, bool forceApprovalPrompt = false)
        {
            ...
            {
                { "client_id", clientID },
                { "scope", YouTubeLiveConnection.ConvertClientScopesToString(scopes) },
                { "response_type", "code" },
        ...
        public static async Task<YouTubeLiveConnection> ConnectViaLocalhostOAuthBrowser(string clientID, string clientSecret, IEnumerable<OAuthClientScopeEnum> scopes, bool forceApprovalPrompt = false, string oauthListenerURL = DEFAULT_OAUTH_LOCALHOST_URL, string successResponse = null)
        {
            Validator.ValidateString(clientID, "clientID");
            Validator.ValidateList(scopes, "scopes");
            ...
            {
                return await YouTubeLiveConnection.ConnectViaAuthorizationCode(clientID, clientSecret, authorizationCode, redirectUrl: oauthListenerURL);
            }
            return null;
        }
        ...
        /// <returns>The YouTubeLiveConnection object</returns>
        public static async Task<YouTubeLiveConnection> ConnectViaAuthorizationCode(string clientID, string clientSecret, string authorizationCode, string redirectUrl = null)
        {
            Validator.ValidateString(clientID, "clientID");
            ...
            return new YouTubeLiveConnection(token);
        }
        ...
        /// <returns>The YouTubeLiveConnection object</returns>
        public static async Task<YouTubeLiveConnection> ConnectViaOAuthToken(OAuthTokenModel token, bool refreshToken = true)
        {
            Validator.ValidateVariable(token, "token");

            YouTubeLiveConnection connection = new YouTubeLiveConnection(token);
            if (refreshToken)
            {
        ...
        private YouTubeLiveConnection(OAuthTokenModel token)
        {
            Validator.ValidateVariable(token, "token");
